...
// ─── Revenue Integration ────────────────────────────────────
const revenue = require('./revenue-engine.js');

// Add revenue tracking to the brain cycle
const originalGetNextAction = getNextAction;
getNextAction = function() {
  const action = originalGetNextAction();
  const forecast = revenue.checkAlerts();
  if (forecast.shortfall > 0) {
    log(`💰 Adjusting strategy due to shortfall: ${forecast.shortfall}`);
    const strategy = revenue.adjustStrategy();
    log(`🧠 New strategy: ${strategy}`);
  }
  return action;
};

// Add revenue forecast to memory
const originalGenerateStrategy = generateStrategy;
generateStrategy = function() {
  const forecast = revenue.getForecast();
  return `Revenue: $${forecast.revenue} | Deals: ${forecast.deals} | Projected: $${forecast.projectedMonthly}`;
};
