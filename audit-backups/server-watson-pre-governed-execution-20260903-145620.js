...
app.use((req,res,next)=>{
  if(req.path === "/__nia-route-test"){
    console.log("🔥 ISOLATED ROUTE HIT");
...
app.use("/api/aios/executive", (req,res,next)=>{ console.log("🔥 EXECUTIVE PRE-AUTH HIT",req.method,req.originalUrl); next(); });
app.get("/api/aios/executive/overview", (req,res)=>{ console.log("🔥 EXECUTIVE ROUTE HIT",req.method,req.originalUrl); res.status(200).json({ok:true,service:"NIA Executive Overview",status:"OPERATIONAL",routeHit:true,generatedAt:new Date().toISOString()}); });
...
loadRoute("./command-center/decision-center-api", "/api/decision-center");

/* NIA GOVERNED COMMAND BRIDGE */
app.post("/api/aios/command", async (req,res)=>{
  try{
    const objective=String(req.body?.command||req.body?.objective||"").trim();
    if(!objective) return res.status(400).json({ok:false,error:"Command required"});

    const reason=require("./aios/core/decision-engine");
    const decision=await Promise.resolve(reason.decide(objective));
    ...
    res.status(200).json({
      ...
      mode:"GOVERNED",
      status:"PENDING_APPROVAL",
      message:"Nia analyzed the command and created a governed proposal.",
      proposal
    });
  }catch(e){
    console.error("NIA COMMAND BRIDGE ERROR:",e);
