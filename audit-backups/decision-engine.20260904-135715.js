...
async function decide(objective, context = {}) {
  ...
  const available_tools = fabric.list();
  const objectiveText = String(objective || '').toLowerCase();
  const preferredTools =
    /outreach|prospect|acquisition|revenue|sales|lead/.test(objectiveText)
      ? ['create_outreach', 'add_prospect', 'create_automation']
      : [];
  const recommended_tool =
    preferredTools
      .map(name => available_tools.find(t =>
        t.name === name && (t.risk === 'low' || t.risk === 'medium')
      )?.name)
      .find(Boolean)
    || available_tools.find(t => t.risk === 'low')?.name
    || null;
